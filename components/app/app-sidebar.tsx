    ...
    return (
      <Sidebar {...props}>
        <SidebarHeader>
          <div
            className={cn(
              "w-full h-16 cursor-pointer bg-zinc-50 flex items-center justify-items-start gap-4 rounded-lg border  px-1 py-2  border-b-zinc-300  border-r-zinc-300"
            )}
            onClick={()=>router.push(`/`)}
          >
            <DatabaseSvg />
          ...
          <NavMain items={SIDEBAR_NAV_MAIN_DATA} />
        </SidebarHeader>
        ...
        <SidebarFooter>
          ...
          >
            ...
            <div className={cn("flex  flex-col items-baseline font-bold")}>
              ...
              <h4
                className={cn("text-muted-foreground text-[12px] inline-block")}
              >
                {planData?.plan_type} plan
                <span
                  className={cn(
                    "text-sm ml-2 text-foreground/80 tracking-tighter "
                  )}
                >
